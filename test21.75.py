...
class OrigamiWidget(QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        # 初始化矩形模型
        self.V, self.F, self.N = self.init_refined_rectangle(n=250)  # 降低密度使其更容易操作
    ...
    def paintGL(self):
        ...
        # 光源跟随相机
        light_pos = [0.0, 0.0, self.camera_distance, 1.0]  # 光源跟随相机
    ...
    def get_3d_coords(self, event):
        """将鼠标位置转换为OpenGL坐标 - 改进版本，考虑相机距离和投影矩阵"""
        width, height = self.width(), self.height()

        # 计算投影矩阵参数
        aspect_ratio = width / height
        fov_y = 45.0  # 视场角度，与initializeGL中设置一致
        fov_y_rad = np.radians(fov_y)

        # 计算在z=0平面上的可视区域大小
        # 使用透视投影公式: tan(fov/2) = (height/2) / distance
        visible_height = 2.0 * self.camera_distance * np.tan(fov_y_rad / 2.0)
        ...
        # 将NDC坐标转换为世界坐标
        world_x = ndc_x * (visible_width / 2.0)
        world_y = ndc_y * (visible_height / 2.0)
        ...
        # 应用逆旋转到坐标
        point = np.array([world_x, world_y, 0])
    ...
    def get_nearest_boundary_point(self, event):
        ...
        if np.any(corner_mask):
            ...
            if min_corner_dist < corner_threshold:  # 角点阈值，使用固定值
    ...
    def apply_fold_transformation(self):
        ...
        # 同时获取对应的法线
        normals = np.array([self.N[i] for i in self.fold_region])
        ...
        # 同样旋转法线（法线只需要旋转，不需要平移）
        rotated_normals = self.rotate_points_around_axis(normals, fold_axis, angle)
        ...
        for i, idx in enumerate(self.fold_region):
            ...
            self.N[idx] = rotated_normals[i].tolist()

        # 更新顶点和法线缓冲区
        if self.vbo is not None and self.nbo is not None:
            vertices = np.array(self.points, dtype=np.float32)
            normals = np.array(self.N, dtype=np.float32)
    ...
    def undo(self):
        ...
        if len(self.history) > 0:
            ...
            # 重新计算法线
            self.N = igl.per_vertex_normals(np.array(self.points), self.F, igl.PER_VERTEX_NORMALS_WEIGHTING_TYPE_ANGLE)
    ...
    def init_refined_rectangle(self, n=20):  # 降低密度使其更容易操作
        ...
        # 添加法线计算
        N = igl.per_vertex_normals(V, F, igl.PER_VERTEX_NORMALS_WEIGHTING_TYPE_ANGLE)
        return V, F, N  # 修改返回值为顶点、面、法线
    ...
    def initializeGL(self):
        ...
        glClearColor(0.9, 0.9, 0.9, 1.0)  # 淡灰色背景
        ...
        # 修改后的材质设置（保持双面材质一致）
        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, [0.6, 0.6, 0.95, 1.0])
        glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, [0.7, 0.7, 1.0, 1.0]) 
...
class MainWindow(QMainWindow):
    ...
    def reset_model(self):
        ...
        # 重新初始化矩形模型
        self.glWidget.V, self.glWidget.F, self.glWidget.N = self.glWidget.init_refined_rectangle(n=250)
