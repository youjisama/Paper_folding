...
class OrigamiWidget(QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        # 初始化矩形模型
        self.V, self.F = self.init_refined_rectangle(n=40)  # 降低密度使其更容易操作
    ...
    def paintGL(self):
        ...
        # 绘制折叠线
        if len(self.fold_line_points) > 0:
            glLineWidth(3.0)  # 加粗折线
            glColor3f(1.0, 0.0, 0.0)  # 红色折线
            glBegin(GL_LINES)
            for i in range(0, len(self.fold_line_points), 2):
                if i + 1 < len(self.fold_line_points):
                    glVertex3fv(self.fold_line_points[i])
                    glVertex3fv(self.fold_line_points[i + 1])
            glEnd()
    ...
    def init_refined_rectangle(self, n=40):  # 降低密度使其更容易操作
