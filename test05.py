    ...
    def mousePressEvent(self, event):
        ...
        if self.mode == "view":
            ...
            if self.fold_point_selection < 2:
                nearest_point_idx = self.get_nearest_boundary_point(event)
                print(f"Fold point selection: {self.fold_point_selection}, Nearest point: {nearest_point_idx}")

                if nearest_point_idx is not None:
                    current_point = self.V[self.boundary_vertices[nearest_point_idx]].tolist()

                    if self.fold_point_selection == 0:
                        self.fold_line_points = [current_point]
                        ...
                        try:
                            ...
                            if self.is_different_edges(first_point_idx, nearest_point_idx):
                                self.fold_line_points.append(current_point)
