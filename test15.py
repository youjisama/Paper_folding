...
# 高级的密度与速度的综合，标注了折线的起点和终点，完美，但是反复折叠的bug依旧存在
class OrigamiWidget(QOpenGLWidget):
    ...
    def initializeGL(self):
        ...
        glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, GL_TRUE)  # 使用局部观察者模型
        glLightModelfv(GL_LIGHT_MODEL_AMBIENT, [0.2, 0.2, 0.2, 1.0])

        # 设置主光源 - 柔和的漫射光
        glLightfv(GL_LIGHT0, GL_POSITION, [0.0, 10.0, 10.0, 0.0])  # 更远的光源位置
        glLightfv(GL_LIGHT0, GL_AMBIENT, [0.2, 0.2, 0.2, 1.0])
        glLightfv(GL_LIGHT0, GL_DIFFUSE, [0.8, 0.8, 0.8, 1.0])
        glLightfv(GL_LIGHT0, GL_SPECULAR, [0.0, 0.0, 0.0, 1.0])  # 禁用镜面反射

        # 设置材质 - 模拟哑光纸张
        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, [0.2, 0.2, 0.2, 1.0])
        glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, [0.9, 0.9, 0.9, 1.0])
        glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, [0.0, 0.0, 0.0, 1.0])
        glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, 0.0)  # 完全哑光
    ...
    def paintGL(self):
        ...
        # 绘制主体
        glBindVertexArray(self.vao)
        glColor4f(0.95, 0.95, 0.95, 1.0)  # 纯白色
