...
class OrigamiWidget(QOpenGLWidget):
    ...
    def mouseMoveEvent(self, event):
        if self.mode == "view":
            ...
            dx = event.x() - self.lastPos.x()
            self.fold_angle += dx * 0.1 * self.fold_direction
            self.fold_angle = max(-180, min(180, self.fold_angle))  # 允许双向折叠
    ...
    def calculate_fold_region(self):
        ...
        for i, vertex in enumerate(self.points):
            ...
            # 考虑折叠方向
            if np.dot(np.cross(line_vector, vertex_vector), [0, 0, 1]) * self.fold_direction > 0:
    ...
    def apply_fold_transformation(self):
        if self.fold_region is not None and len(self.fold_line_points) == 2:
            ...
            for i in self.fold_region:
                ...
                rotated = self.rotate_around_axis(translated, fold_axis, np.radians(abs(self.fold_angle)))
